...
public class FieldLevelEncryption implements CryptographyInterceptor {
        ...
        try {
            ...
            for (String fieldToEncrypt : config.fieldsToEncrypt) {
                if(smartMap.containsKey(fieldToEncrypt)) {
                    ...
                    SecretKey secretKey = CryptUtil.generateSecretKey(config.symmetricCipher, config.symmetricKeysize, config.digestAlgorithm);
                    ...
                    byte[] certificateFingerprint = CryptUtil.generateFingerprint(config.digestAlgorithm, this.publicCertificate);
            ...
            if (this.privateKey != null) {
                for (String fieldToDecrypt : config.fieldsToDecrypt) {
                    if (smartMap.containsKey(fieldToDecrypt)) {
                        ...
                        if (enclosingBlock.containsKey(encryptedDataMapField) && enclosingBlock.containsKey(config.encryptedKeyFiledName)) {
    ...
    public boolean equals(Object o) {
        ...
        MDESCryptography otherMdes = (MDESCryptography) o;
